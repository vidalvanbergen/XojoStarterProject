#tag DesktopWindow
Begin DesktopWindowPro wndDebugLog
   Backdrop        =   0
...
#tag WindowCode
	#tag Event
		Function ConstructToolbar(Toolbar as NSToolbar) As Boolean
		  ...
		  Toolbar.DefaultItems = Array( NSToolbarFlexibleSpaceItemIdentifier, pbClear.Name )
		  ...
		  // Add toolbar items
		  Toolbar.AddControl( pbClear, "Clear", "Clear List" ) // Simplified method to add customtoolbar items with controls
	...
	#tag Event
		Sub Settings()
		  'self.PrefFrameName = "wndDebug" // Allow window position to be saved.
		  self.PrefToolbarName = "NSToolbarDebug" // Activate toolbar support.
	...
	#tag Method, Flags = &h0
		Sub Send(s as String)
		  lbMsgs.AddRow( s )
		  lbMsgs.SelectedRowIndex = lbMsgs.LastRowIndex
		End Sub
	#tag EndMethod
