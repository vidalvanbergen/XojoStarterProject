...
Protected Module Helpers
	...
	#tag Method, Flags = &h0
		Function CleanStack(Extends error as RuntimeException) As string()
		  Return Helpers.CleanStack(error)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function CleanStack(error as RuntimeException) As String()
		  ...
		  // we must return some sort of array (even if empty), otherwise Xojo will return a "nil" array, causing a crash when trying to use the array.
	...
	#tag Method, Flags = &h0
		Sub Log(Message as String, Type as Integer = 0)
		  ...
		  if AppSettings.ExperimentalMode then 'or DebugBuild then
		    // DReport
		    Select case Type
		    case 0 // Debug
		  ...
		  'if DebugBuild and TargetDesktop then
	...
	#tag Method, Flags = &h1
		Protected Function macOSName() As String
		  ...
		  #If TargetMacOS
		    ...
		    elseif IsSequoia then // OS 15
		      os = "Sequoia"
		    elseif IsSonoma then // OS 14
		      os = "Sonoma"
		    elseif IsVentura then // OS 13
		      os = "Ventura"
		    elseif IsMonterey then // OS 12
		      os = "Monterey"
		      
		    elseif False then // 10.17
		      
		    elseif False then // 10.16
		      
		    elseif IsCatalina then // 10.15
		      os = "Catalina"
		    elseif IsMojave then // 10.14
		      os = "Mojave"
		    elseif IsHighSierra then // 10.13
		      os = "High Sierra"
		    elseif IsSierra then // 10.12
		      os = "Sierra"
		    elseif IsElCapitan then // 10.11
		      os = "El Capitan"
		    elseif IsYosemite then // 10.10
		      os = "Yosemite"
		    elseif IsMavericks then // 10.9
		      os = "Mavericks"
		      
		      // OS versions lower than this are basically deprecated.
		    elseif IsMountainLion then // 10.8
		      os = "Mountain Lion"
		    elseif IsLion then // 10.7
		      os = "Lion"
		    elseif IsSnowLeopard then // 10.6
		      os = "Snow Leopard"
		    elseif IsLeopard then // 10.5
		      os = "Leopard"
		    elseif IsTiger then // 10.4
		      os = "Tiger"
		    elseif IsPanther then //10.3
		      os = "Panther"
		    else
		      os = "Unknown"
		    end if
	...
	#tag Method, Flags = &h1
		Protected Function OSVersion() As String
		  ...
		  #If TargetMacOS
		    ...
		    Return "MacOS X "+os+" "+ SystemVersionAsString
