...
Inherits DesktopMenuItem
	...
	#tag Method, Flags = &h21
		Private Shared Function GetNSMenu(m as DesktopMenuItem) As Ptr
		  #if targetCocoa
	...
	#tag Method, Flags = &h21
		Private Shared Function GetNSMenuItem(m as DesktopMenuItem) As Ptr
		  #if targetCocoa
		    declare function itemWithTitle lib CocoaLib selector "itemWithTitle:" (id as Ptr, aString as CFStringRef) as Ptr
		    
		    dim parent as DesktopMenuItem = GetParentOf(m)
	...
	#tag Method, Flags = &h21
		Private Shared Function GetParentOf(theItem as DesktopMenuItem) As DesktopMenuItem
		  ...
		  dim parentStack(-1) as DesktopMenuItem 'MenuItem
		  parentStack.Append App.MenuBar
		  
		  while UBound(parentStack) > -1
		    dim parent as DesktopMenuItem = parentStack.Pop
		    for i as Integer = 0 to parent.Count - 1
		      dim childItem as DesktopMenuItem = parent.MenuAt(i)
