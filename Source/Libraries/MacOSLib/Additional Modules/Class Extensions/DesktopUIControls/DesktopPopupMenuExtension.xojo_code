#tag Module
Protected Module DesktopPopupMenuExtension
	#tag Method, Flags = &h0
		Sub AddRowWithTagAndPicture(extends p as DesktopPopupMenu, Name as String, Tag as Variant, Ico as Picture)
		  ...
		  p.AddRow Name
		  p.RowTagAt( p.LastAddedRowIndex ) = Tag
		  if Ico <> nil then
		    p.Icon(   p.LastAddedRowIndex ) = NSImage.CreateFromPicture( Ico )
		  end if
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h0
		Function ArrowPosition(Extends p as DesktopPopupMenu) As NSPopupArrowPosition
	...
	#tag Method, Flags = &h0
		Sub ArrowPosition(extends p as DesktopPopupMenu, assigns Value as NSPopupArrowPosition)
	...
	#tag Method, Flags = &h0
		Function AutoenablesItems(Extends p as DesktopPopupMenu) As Boolean
	...
	#tag Method, Flags = &h0
		Sub AutoenablesItems(Extends p as DesktopPopupMenu, assigns value as Boolean)
		  //# Sets whether the PopupMenu automatically enables and disables its items every time a user event occurs.
		  
		  #if TargetCocoa then
		    declare sub setAutoenablesItems lib CocoaLib selector "setAutoenablesItems:" (obj_id as Ptr, value as Boolean)
	...
	#tag Method, Flags = &h0
		Function BezelStyle(extends p as DesktopPopupMenu) As NSBezelStyle
	...
	#tag Method, Flags = &h0
		Sub BezelStyle(extends p as DesktopPopupMenu, assigns Value as NSBezelStyle)
		  //# Sets the appearance of the border, if the PopupMenu has one.
		  
		  #if TargetCocoa then
		    declare sub setBezelStyle lib CocoaLib selector "setBezelStyle:" (obj_id as Ptr, inFlag as NSBezelStyle)
	...
	#tag Method, Flags = &h0
		Function Bordered(Extends p as DesktopPopupMenu) As Boolean
	...
	#tag Method, Flags = &h0
		Sub Bordered(Extends p as DesktopPopupMenu, Assigns value as Boolean)
	...
	#tag Method, Flags = &h0
		Function ControlSize(Extends p as DesktopPopupMenu) As NSControlSize
	...
	#tag Method, Flags = &h0
		Sub ControlSize(Extends p as DesktopPopupMenu, Assigns value as NSControlSize)
	...
	#tag Method, Flags = &h0
		Sub Icon(extends p as DesktopPopupMenu, index as Integer, setTemplate as Boolean = False, assigns value as NSImage)
		  #if TargetCocoa
	...
	#tag Method, Flags = &h0
		Function IndexOfItemWithTitle(extends p as DesktopPopupMenu, Title as String) As Integer
		  //# Returns the index of the item with the specified title.
		  
		  #if TargetCocoa then
		    declare function indexOfItemWithTitle lib CocoaLib selector "indexOfItemWithTitle:" (obj_id as Ptr, inTitle as CFStringRef) as Integer
	...
	#tag Method, Flags = &h0
		Function isBordered(extends p as DesktopPopupMenu) As Boolean
	...
	#tag Method, Flags = &h0
		Sub isBordered(extends p as DesktopPopupMenu, assigns Value as Boolean)
		  //# Places or removes a border on the PopupMenu and redraws the PopupMenu.
		  
		  #if TargetCocoa then
		    declare sub setBordered lib CocoaLib selector "setBordered:" (obj_id as Ptr, inFlag as Boolean)
	...
	#tag Method, Flags = &h0
		Function isTransparent(extends p as DesktopPopupMenu) As Boolean
	...
	#tag Method, Flags = &h0
		Sub isTransparent(extends p as DesktopPopupMenu, assigns Value as Boolean)
		  //# Sets whether the PopupMenu is transparent.
		  
		  //@ A transparent button never draws itself, but it receives mouse-down events and tracks the mouse properly.
		  
		  #if TargetCocoa then
		    declare sub setTransparent lib CocoaLib selector "setTransparent:" (obj_id as Ptr, inFlag as Boolean)
	...
	#tag Method, Flags = &h21
		Private Function PopUpButtonCell(Extends p as DesktopPopupMenu) As Ptr
	...
	#tag Method, Flags = &h0
		Function PreferredEdge(extends p as DesktopPopupMenu) As NSRectEdge
	...
	#tag Method, Flags = &h0
		Sub PreferredEdge(extends p as DesktopPopupMenu, assigns Value as NSRectEdge)
		  //# Sets the edge of the PopupMenu next to which the menu should appear under restrictive screen conditions.
		  
		  #if TargetCocoa
		    declare sub setPreferredEdge lib CocoaLib selector "setPreferredEdge:" (obj_id as Ptr, inFlag as NSRectEdge)
	...
	#tag Method, Flags = &h0
		Function PullsDown(extends p as DesktopPopupMenu) As Boolean
		  //# Returns a Boolean value indicating the behavior of the PopupMenu's menu.
		  
		  #if TargetCocoa then
		    declare function pullsDown lib CocoaLib selector "pullsDown" (obj_id as Ptr) as Boolean
	...
	#tag Method, Flags = &h0
		Sub PullsDown(extends p as DesktopPopupMenu, assigns Value as Boolean)
		  //# Sets whether the PopupMenu behaves as a pull-down or pop-up menu.
		  
		  #if TargetCocoa then
		    declare sub setPullsDown lib CocoaLib selector "setPullsDown:" (obj_id as Ptr, inFlag as Boolean)
	...
	#tag Method, Flags = &h0
		Sub RemoveRow(extends p as DesktopPopupMenu, Title as String)
		  //# Removes the item with the specified title from the menu.
		  
		  #if TargetCocoa then
		    declare sub removeItemWithTitle lib CocoaLib selector "removeItemWithTitle:" (obj_id as Ptr, inTitle as CFStringRef)
	...
	#tag Method, Flags = &h0
		Sub SelectItemWithTitle(extends p as DesktopPopupMenu, Title as String)
		  //# Selects the item with the specified title.
		  
		  //@ Doesn't activate the 'Action' event.
		  
		  #if TargetCocoa then
		    declare sub selectItemWithTitle lib CocoaLib selector "selectItemWithTitle:" (obj_id as Ptr, inTitle as CFStringRef)
	...
	#tag Method, Flags = &h0
		Sub SetTitle(extends p as DesktopPopupMenu, Title as String)
		  ...
		  #if TargetCocoa then
		    declare sub setTitle lib CocoaLib selector "setTitle:" (obj_id as Ptr, inTitle as CFStringRef)
	...
	#tag Method, Flags = &h0
		Function ShowsBorderOnlyWhileMouseInside(extends p as DesktopPopupMenu) As Boolean
	...
	#tag Method, Flags = &h0
		Sub ShowsBorderOnlyWhileMouseInside(extends p as DesktopPopupMenu, assigns Value as Boolean)
		  //# Sets whether the PopupMenu's border is displayed only when the cursor is over the button.
		  
		  #if TargetCocoa then
		    declare sub setShowsBorderOnlyWhileMouseInside lib CocoaLib selector "setShowsBorderOnlyWhileMouseInside:" (obj_id as Ptr, inFlag as Boolean)
	...
	#tag Method, Flags = &h0
		Sub SynchronizeTitleAndSelectedItem(Extends p as DesktopPopupMenu)
		  //# Synchronizes the the pop-up button’s displayed item with the currently selected menu item.
		  
		  #if TargetCocoa
		    declare sub synchronizeTitleAndSelectedItem lib CocoaLib selector "synchronizeTitleAndSelectedItem" (obj_id as Ptr)
	...
	#tag Method, Flags = &h0
		Function Transparent(extends p as DesktopPopupMenu) As Boolean
	...
	#tag Method, Flags = &h0
		Sub Transparent(extends p as DesktopPopupMenu, Assigns value as Boolean)
