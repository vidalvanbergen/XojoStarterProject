...
Protected Module XMLExtensions
	...
	#tag Method, Flags = &h0
		Sub Indent(extends parent as XmlNode, level as Integer, IndentCharacters as String = "  ")
		  Dim s, tab As String
		  Dim HasChildren as Boolean
		  Dim Doc as XmlDocument = parent.OwnerDocument
		  
		  tab = IndentCharacters // chr(9)
		  s = chr(10)
		  ...
		  For i as Integer = parent.ChildCount -1 DownTo 0
		    If parent.Child( i ).Type = XMLNodeType.ELEMENT_NODE Then
		      parent.Child( i ).Indent( level + 1 )
		      parent.Insert( Doc.CreateTextNode( s + tab ), parent.Child( i ) )
		      HasChildren = True
		    elseif parent.Child( i ).Type = XMLNodeType.COMMENT_NODE then
		      parent.Child( i ).Indent( level + 1 )
		      parent.Insert( Doc.CreateTextNode( s + tab ), parent.Child( i ) )
		      HasChildren = True
		    End If
