	...
	#tag Method, Flags = &h21
		Private Sub RunScript()
		  ...
		  Exception err As RuntimeException
		    MessageBox "Could not complete update - " + err.Message
	...
	#tag Method, Flags = &h21
		Private Function SaveScript(script As String, tempFolder As FolderItem) As FolderItem
		  ...
		  if bs.LastErrorCode <> 0 then
		    MessageBox "Error writing script file: " + str( bs.LastErrorCode )
		    scriptFile = nil
		  end if
		  bs.Close
		  bs = nil
		  ...
		  Exception err as IOException
		    MessageBox "Error writing script file: " + Str( err.ErrorNumber )
		    scriptFile = Nil
		End Function
	#tag EndMethod
